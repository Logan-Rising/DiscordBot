...
module.exports = {
	data: new SlashCommandBuilder()
        ...
        .addStringOption(option => option.setName('title').setDescription('Title of the reaction role group').setRequired(false))
	...
	async execute(interaction, firedb) {
        ...
        if (!role1 && !role2 && !role3 && !role4 && !role5) {
            await interactionfunctions.Reply(firedb, interaction, { content: 'Must include at least one role for the reaction role message'});
            return;
        }
        ...
        await reactionmessagefunctions.StoreReactionMessage(firedb, interaction.guild.id, interaction.channel.id, message.id, role1 ? role1.id : '', role2 ? role2.id : '', 
            role3 ? role3.id : '', role4 ? role4.id : '', role5 ? role5.id : '');
